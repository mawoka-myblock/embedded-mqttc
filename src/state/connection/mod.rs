...
use mqttrs2::{
    decode_slice_with_len, encode_slice, Connack, Connect, LastWill, Packet, Protocol, Suback,
    Subscribe,
};

use crate::{
    buffer::{MappedBufferRef, StackBufferCell},
    fmt::Debug2Format,
    state::pid::next_pid,
    AutoSubscribe, ClientConfig, MqttError,
};
...
impl<'a, 'l, M: RawMutex, NETWORK, DNS, const BUFFER_SIZE: usize>
    TcpConnectionState<'a, 'l, M, NETWORK, DNS, BUFFER_SIZE>
where
    NETWORK: TcpConnect,
    DNS: Dns,
{
    ...
    fn try_read_packet(
        &self,
    ) -> Result<Option<MappedBufferRef<'_, Packet<'_>, BUFFER_SIZE>>, MqttError> {
        let buffer = self.recv_buffer.borrow();
        let is_max_len = buffer.is_max_len();

        let result = buffer
            .try_map_maybe(|buf| decode_slice_with_len(buf))
    ...
    async fn send_all_intern(
        &self,
        connection: &mut NETWORK::Connection<'_>,
    ) -> Result<(), MqttError> {
        let mut send_buffer = self.send_buffer.borrow();
    ...
    pub async fn send_all(&self) -> Result<(), MqttError> {
        let mut connection = self.connection.borrow_mut();
        let connection = connection.as_mut().unwrap();
    ...
    async fn subscribe_auto_subscribes(&self) -> Result<(), MqttError> {
        for chunk in self.config.auto_subscribes.chunks(5) {
            ...
            let request = Subscribe { pid, topics };
            ...
            let suback = match next_packet.deref() {
                ...
                unexpected_packet => {
                    error!(
                        "got unexpected packet {} while waiting for suback",
                        unexpected_packet.get_type()
                    );
                    return Err(MqttError::ConnackError);
                }
            };
        ...
        for (suback, requestes_topic) in suback.return_codes.iter().zip(auto_subscribes.iter()) {
            match suback {
                mqttrs2::SubscribeReturnCodes::Success(qos) if *qos == requestes_topic.qos => {
                    info!(
                        "successfully auto subscribes to {} with {}",
                        &requestes_topic.topic, qos
                    );
                }
                mqttrs2::SubscribeReturnCodes::Success(qos) => {
                    warn!(
                        "autosubscribes to {} with different qos: requested {} but got {}",
                        &requestes_topic.topic, requestes_topic.qos, qos
                    );
                }
    ...
    async fn process_connack(&self, connack: &Connack) -> Result<(), MqttError> {
        match connack.code {
            ...
            mqttrs2::ConnectReturnCode::RefusedProtocolVersion
            | mqttrs2::ConnectReturnCode::RefusedIdentifierRejected
            | mqttrs2::ConnectReturnCode::ServerUnavailable => {
                error!("connack returned error: {}", connack.code);
                self.inner.sender().send(ConnectionStateValue::Error);
                Err(MqttError::ConnackError)
            }
            mqttrs2::ConnectReturnCode::BadUsernamePassword
            | mqttrs2::ConnectReturnCode::NotAuthorized => {
    ...
    async fn send_receive(&self) -> Result<(), MqttError> {
        ...
        while send_buffer.has_remaining_len() {
            ...
            network_flush!(connection);
            debug!(
                "write {} bytes to network, {} remaining",
                bytes_sent,
                send_buffer.reaable_data().len()
            );
...
impl<'a, 'l, M: RawMutex, NETWORK, DNS, const BUFFER_SIZE: usize> ConnectionState
    for TcpConnectionState<'a, 'l, M, NETWORK, DNS, BUFFER_SIZE>
where
    NETWORK: TcpConnect,
    DNS: Dns,
{
    ...
    async fn connect(&self) -> Result<(), MqttError> {
        assert!(
            self.inner.try_get() == None
                || self.inner.try_get() == Some(ConnectionStateValue::Error)
        );
        ...
        let connack = match next_packet.deref() {
            ...
            unexpected_packet => {
                error!(
                    "got unexpected packet {} while waiting for connack",
                    unexpected_packet.get_type()
                );
                return Err(MqttError::ConnackError);
            }
    ...
    async fn await_connected(&self) {
        self.inner
            .receiver()
            .unwrap()
            .get_and(|value| *value == ConnectionStateValue::Connected)
            .await;
    }
    ...
    async fn run_io_nonblocking(
        &self,
    ) -> Result<Option<impl Deref<Target = Packet<'_>>>, MqttError> {
        assert_eq!(self.inner.try_get(), Some(ConnectionStateValue::Connected));
...
mod tests {
    ...
    use crate::{
        state::connection::{
            test::{TestDns, TestTcpConnect},
            ConnectionState, ConnectionStateValue, TcpConnectionState,
        },
        ClientConfig, Host,
    };
    ...
    fn test_connect() {
        ...
        let connection_state: TcpConnectionState<'_, '_, CriticalSectionRawMutex, _, _, 1024> =
            TcpConnectionState::new(&tcp, dns, None, client_config.clone());
        ...
        let mut connect_future = unsafe { Pin::new_unchecked(&mut connect_future) };

        // Send connect packet to network, now waiting for connack
        assert_pending(connect_future.as_mut());
        assert_eq!(
            connection_state_receiver.try_changed(),
            Some(ConnectionStateValue::ConnectSent)
        );
    ...
    fn connect<M: RawMutex, const BUFFER: usize>(
        state: &TcpConnectionState<'_, '_, M, TestTcpConnect, TestDns, BUFFER>,
        tcp: &TestTcpConnect,
    ) {
    ...
    fn test_disconnect() {
        ...
        assert_eq!(
            connection_state.inner.dyn_receiver().unwrap().try_get(),
            Some(ConnectionStateValue::Disconnected)
        );
    }
}
